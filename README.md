...
```sh
pnpm install
pnpm typecheck   # tsc -b across the family
pnpm test        # host-side vitest suites (browser-surface specs are typechecked reference material)
pnpm build       # tsc + tsdown bundles per package
```

The browser-surface specs under `packages/client/ui-pet/tests` are typechecked here but executed inside the harness workspace, because the npm-published client packages ship window-loader bundles rather than source-plane ESM.
...
[MIT](LICENSE) —— DeepSeek Harness 的衍生作品。
