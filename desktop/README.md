...
Electron 桌面壳：双击启动后自动拉起 harness Web 服务器（子进程），在窗口内使用 Web UI，关窗即退出。

## 前置条件

- 仓库已安装依赖并构建：在仓库根执行 `pnpm install` 和 `pnpm run build`
  （Web UI 由构建产物 `apps/web/dist` 提供，未构建则服务器无法出页面）
...
产出 `dist/DeepSeek Harness 0.1.0.exe` 便携版。注意：便携版只包含桌面壳本身，仍需要仓库检出和系统 Node 才能运行（本机自用场景）。
