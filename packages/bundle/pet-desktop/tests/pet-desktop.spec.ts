 ...
 *   - `@deepseek-ai/dsh-desktop-companion` is a real dependency (not a peer),
...
describe('dsh-pet-desktop bundle', () => {
  ...
  it('depends on @deepseek-ai/dsh-desktop-companion so profiles install the service', () => {
    ...
    expect(manifest.dependencies).toHaveProperty('@deepseek-ai/dsh-desktop-companion')
    expect(manifest.peerDependencies).not.toHaveProperty('@deepseek-ai/dsh-desktop-companion')
  })
  ...
  it('loads the desktop-companion plugin before pet so pet can register its window', () => {
    ...
    expect(companionRow?.name).toBe('@deepseek-ai/dsh-desktop-companion')
  })
})
