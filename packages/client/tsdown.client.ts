 ...
 * The `pet`/`pet-compat` pair is the pet domain's pure library layer (frozen
 * constants, free functions, one wire-irrelevant Error class); they are library
 * dependencies of the assembled pet plugins, never client plugins themselves.
 * Everything else under @deepseek-ai/* is either a module-table entry
 * (external) or a leak the purity gate rejects.
 */
export const INLINE_SAFE = /^@deepseek-ai\/dsh-(host-apiproxy|file-reference|session|llm|tools|brand|pet-compat|pet)(\/|$)/
